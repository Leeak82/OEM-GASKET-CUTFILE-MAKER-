...
function setCutfileVerified(value) {
  CUTFILE_VERIFIED = Boolean(value);
  const checkbox = safeEl("measurementVerified");
  if (checkbox && checkbox.checked !== CUTFILE_VERIFIED) checkbox.checked = CUTFILE_VERIFIED;

  const status = safeEl("cutfileStatus");
  const download = safeEl("downloadSvgBtn");
  if (status) {
    status.textContent = CUTFILE_VERIFIED
      ? "Verified measurements — SVG export enabled."
      : "Preview only — not verified for cutting.";
    status.classList.toggle("ready", CUTFILE_VERIFIED);
  }
  if (download) download.disabled = !CUTFILE_VERIFIED;

  const svgOutput = safeEl("svgOutput");
  if (svgOutput?.innerHTML.trim()) renderSVG();
}

function invalidateCutfileVerification() {
  if (CUTFILE_VERIFIED) setCutfileVerified(false);
}

function partUsesApproximateGeometry(part) {
  if (!part?.geometry) return true;
  const hasExactOutline = Boolean(part.geometry.outlinePath?.trim());
  const hasExactHoles = Array.isArray(part.geometry.holePattern) && part.geometry.holePattern.length > 0;
  return !(hasExactOutline && hasExactHoles && part.geometry.geometryStatus === "verified_exact");
}
...
function resolvePatternInfo(part) {
  if (!part?.geometry?.patternRef) {
    return {
      name: "Direct Geometry",
      source: partUsesApproximateGeometry(part) ? "Reference / unverified" : "Verified exact geometry"
    };
  }
  ...
  return {
    name: `${pattern.name || part.geometry.patternRef} (PREVIEW ONLY)`,
    source: `${pattern.source || "Pattern Library"} — generic, not OEM cut geometry`
  };
}
...
function renderSVG() {
  ...
  const warningMarkup = CUTFILE_VERIFIED
    ? ""
    : `<text x="${width / 2}" y="${height / 2}" text-anchor="middle" font-size="${Math.max(8, Math.min(width, height) / 12)}" fill="red" opacity="0.75">PREVIEW ONLY - VERIFY MEASUREMENTS</text>`;

  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="${width}mm" height="${height}mm" viewBox="0 0 ${width} ${height}">
      <path d="${outlinePath}" fill="none" stroke="black" stroke-width="0.5" />
      ${holesMarkup}
      ${warningMarkup}
    </svg>
  `;
...
function downloadSVG() {
  if (!CUTFILE_VERIFIED) {
    alert("Download blocked: verify the real-world measurements and hole locations first.");
    return;
  }
...
function processScan() {
  ...
  const pxPerUnit = Number(getValue("scanScale"));
  const minHoleArea = Math.max(10, Number(getValue("scanMinHoleSize")) || 80);

  if (!Number.isFinite(pxPerUnit) || pxPerUnit <= 0) {
    setScanStatus("Calibration required: enter measured pixels per mm before processing dimensions.");
    return;
  }
  ...
  setCutfileVerified(false);
  setScanStatus(`Scan complete: ${widthUnits} x ${heightUnits}, ${scaledHoles.length} holes detected. Calibrate and verify against a real measurement before export.`);
}
...
function attachEvents() {
  ...
  safeEl("measurementVerified")?.addEventListener("change", (event) => {
    setCutfileVerified(event.target.checked);
  });
  ["width", "height", "manualHoles"].forEach((id) => {
    safeEl(id)?.addEventListener("input", invalidateCutfileVerification);
  });
